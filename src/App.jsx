...
const PROFILE = {
  ...
  tagline: "Driving innovation in user-centric mobile solutions across React Native, Android, iOS and cloud-native backends.",
...
const TECH = {
  ...
  tooling: [
    "Keycloak", "OAuth2.0", "Firebase", "Dynatrace", "UserExperior", "pCloudy",
    "MoEngage", "Adobe Analytics", "AEM", "AppsFlyer", "JIRA", "Bitbucket",
  ],
};
...
const badges = (arr) => (
  <div className="flex flex-wrap gap-2">
    {arr.map((t) => (
      <span key={t} className="px-3 py-1 rounded-full bg-accent text-sm">
        {t}
      </span>
    ))}
  </div>
...
function Stat({ value, label }) {
  return (
    <motion.div className="p-5 rounded-2xl bg-card shadow-sm border"
...
function NavItem({ to, children, active, onClick }) {
  return (
    <a href={to} onClick={(e) => { if (onClick) { e.preventDefault(); onClick(); } }}
...
export default function ResumeSite() {
  ...
  useEffect(() => { const id = setInterval(() => setWordIndex(i => (i+1) % heroWords.length), 1800); return () => clearInterval(id); }, [heroWords.length]);
    ...
    <main className="min-h-screen bg-background text-foreground selection:bg-primary/20">
      <header className="sticky top-0 z-40 border-b backdrop-blur supports-[backdrop-filter]:bg-background/70">
        <div className="max-w-6xl mx-auto px-4 md:px-6 h-16 flex items-center justify-between">
          <a href="#top" className="font-bold tracking-tight text-lg">Nitin Kalokhe</a>
          <nav className="hidden md:flex items-center gap-1 text-sm">
            <NavItem to="#experience" active={currentSection==='experience'} onClick={() => smoothTo('experience')}>Experience</NavItem>
            <NavItem to="#portfolio" active={currentSection==='portfolio'} onClick={() => smoothTo('portfolio')}>Portfolio</NavItem>
            <NavItem to="#tech" active={currentSection==='tech'} onClick={() => smoothTo('tech')}>Tech</NavItem>
            <NavItem to="#leadership" active={currentSection==='leadership'} onClick={() => smoothTo('leadership')}>Leadership</NavItem>
            <NavItem to="#awards" active={currentSection==='awards'} onClick={() => smoothTo('awards')}>Awards</NavItem>
            <NavItem to="#education" active={currentSection==='education'} onClick={() => smoothTo('education')}>Education</NavItem>
            <a href={PROFILE.resumePDF} className="ml-2 inline-flex items-center gap-2 rounded-xl px-4 py-2 border font-medium hover:bg-accent" download>
              <Download className="w-4 h-4" /> Resume
            </a>
          </nav>
          <button onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} className="md:ml-4 rounded-xl border px-3 py-2 text-sm hover:bg-accent" aria-label="Toggle theme">
            {theme === 'dark' ? '☀️' : '🌙'}
          </button>
        </div>
      </header>
      ...
      <section id="top" className="relative overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto px-6 pt-16 md:pt-24 pb-12">
          <div className="grid md:grid-cols-2 gap-10 items-center">
            <div>
              <motion.h1 initial={{ y: 16, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ type: 'spring', stiffness: 90 }}
                className="text-3xl md:text-5xl font-extrabold tracking-tight">{PROFILE.name}</motion.h1>
              ...
              <div className="mt-6 text-lg">
                ...
                <AnimatePresence mode="wait">
                  <motion.span key={wordIndex} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.35 }} className="font-semibold">
                    {heroWords[wordIndex]}
                  </motion.span>
                </AnimatePresence>
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <a href={`mailto:${PROFILE.email}`} className="inline-flex items-center gap-2 rounded-2xl border px-4 py-2 hover:bg-accent"><Mail className="w-4 h-4" /> {PROFILE.email}</a>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.6 }} className="relative">
              <div className="rounded-3xl border bg-card p-6 shadow-sm">
                ...
                <ul className="mt-3 space-y-3 text-sm leading-relaxed">
                  <li className="flex items-start gap-2"><ChevronRight className="w-4 h-4 mt-1" />Optimized API response times ~500ms → ~20ms with Redis caching.</li>
                  <li className="flex items-start gap-2"><ChevronRight className="w-4 h-4 mt-1" />Event-driven email service using Kafka for cross-service scale.</li>
                  <li className="flex items-start gap-2"><ChevronRight className="w-4 h-4 mt-1" />AWS-hosted microservices (EB, RDS, CloudWatch) for reliability.</li>
                </ul>
                <a href="#portfolio" className="mt-6 inline-flex items-center gap-2 text-primary font-medium">Explore portfolio <ArrowRight className="w-4 h-4" /></a>
              </div>
            </motion.div>
          </div>
        </div>
      ...
      <Section id="experience" title="Experience" icon={<BriefcaseBusiness className="w-6 h-6" />}>
        <div className="grid md:grid-cols-3 gap-6">
          {ROLES.map((r) => (
            <motion.div key={r.org} className="rounded-2xl border bg-card p-5 shadow-sm"
              initial={{ y: 20, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} viewport={{ once: true }}
              transition={{ type: 'spring', stiffness: 100 }}>
              <div className="flex items-center gap-3">
                <Building2 className="w-5 h-5" />
                <div><div className="font-semibold">{r.org}</div>
                  <div className="text-xs text-muted-foreground">{r.period}</div></div>
              </div>
              <div className="mt-3 font-medium">{r.title}</div>
              <ul className="mt-3 space-y-2 text-sm text-muted-foreground">
                {r.bullets.map((b, i) => (<li key={i} className="flex gap-2"><ChevronRight className="w-4 h-4" />{b}</li>))}
              </ul>
            </motion.div>
          ))}
        </div>
      ...
      <Section id="portfolio" title="Portfolio & Apps" icon={<Rocket className="w-6 h-6" />}>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {APPS.map((a) => (
            <motion.a key={a.name} href={a.href} target="_blank"
      ...
      <Section id="tech" title="Tech Stack" icon={<Cpu className="w-6 h-6" />}>
        <div className="space-y-6">
          <div><div className="mb-2 text-sm font-semibold">Mobile</div>{badges(TECH.mobile)}</div>
          <div><div className="mb-2 text-sm font-semibold">Backend</div>{badges(TECH.backend)}</div>
          <div><div className="mb-2 text-sm font-semibold">Tooling & Services</div>{badges(TECH.tooling)}</div>
        </div>
      </Section>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <motion.div className="rounded-2xl border bg-card p-5 shadow-sm" initial={{ y: 20, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} viewport={{ once: true }}>
            ...
            <ul className="text-sm text-muted-foreground space-y-2">
              <li className="flex gap-2"><ChevronRight className="w-4 h-4" />Strategy, roadmap, resourcing & OKRs</li>
              <li className="flex gap-2"><ChevronRight className="w-4 h-4" />Architecture reviews, design governance, security posture</li>
              <li className="flex gap-2"><ChevronRight className="w-4 h-4" />Quality gates, performance budgets, release management</li>
              <li className="flex gap-2"><ChevronRight className="w-4 h-4" />Vendor & stakeholder management, cross-functional alignment</li>
            </ul>
          </motion.div>
          <motion.div className="rounded-2xl border bg-card p-5 shadow-sm" initial={{ y: 20, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} viewport={{ once: true }}>
            <div className="font-semibold mb-2">Signature projects</div>
            <ul className="text-sm text-muted-foreground space-y-2">
              <li className="flex gap-2"><ChevronRight className="w-4 h-4" />Super-app foundation with multi-product flavors</li>
              <li className="flex gap-2"><ChevronRight className="w-4 h-4" />Microservices on AWS EB + RDS, evented email via Kafka</li>
              <li className="flex gap-2"><ChevronRight className="w-4 h-4" />Analytics & growth stack (MoEngage, AppsFlyer, Adobe)</li>
            </ul>
          </motion.div>
        </div>
      </Section>
      ...
      <Section id="awards" title="Awards" icon={<Trophy className="w-6 h-6" />}>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {AWARDS.map((a) => (
            <motion.div key={a.name} className="rounded-2xl border bg-card p-5 shadow-sm"
              initial={{ y: 20, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} viewport={{ once: true }}>
              <div className="flex items-center gap-3">
                <Award className="w-5 h-5" />
                <div><div className="font-semibold">{a.name}</div>
                  <div className="text-xs text-muted-foreground">{a.org}</div></div>
              </div>
            </motion.div>
          ))}
        </div>
      ...
      <Section id="education" title="Education" icon={<Star className="w-6 h-6" />}>
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          {EDUCATION.map((e) => (
            <motion.div key={e.degree} className="rounded-2xl border bg-card p-5 shadow-sm"
              initial={{ y: 20, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} viewport={{ once: true }}>
        ...
        <div className="max-w-6xl mx-auto px-6">
          <div className="rounded-3xl border bg-card p-8 md:p-12 shadow-sm text-center">
            ...
            <div className="mt-6 flex flex-wrap justify-center gap-3">
              <a href={`mailto:${PROFILE.email}`} className="inline-flex items-center gap-2 rounded-2xl border px-5 py-3 hover:bg-accent"><Mail className="w-4 h-4" /> Email me</a>
              <a href={PROFILE.linkedin} target="_blank" className="inline-flex items-center gap-2 rounded-2xl border px-5 py-3 hover:bg-accent"><Linkedin className="w-4 h-4" /> Connect on LinkedIn</a>
              <a href={PROFILE.resumePDF} download className="inline-flex items-center gap-2 rounded-2xl border px-5 py-3 hover:bg-accent"><Download className="w-4 h-4" /> Download Résumé</a>
            </div>
          </div>
        </div>
      </section>
